...
function countProfit(shoppers) {
    ...
    let productProfitInfoList = listBarang.map(array => ({
        product : array[0],
        shoppers : [],
        leftOver : array[2],
        totalProfit : 0,
    ...
    for( let shopper of shoppers){
        let pinfo = productProfitInfoList.find((p) => p.product === shopper.product);
        if(pinfo && pinfo.leftOver >= shopper.amount){
            pinfo.shoppers.push(shopper.name);
            pinfo.leftOver -= shopper.amount;
            pinfo.totalProfit += (shopper.amount * pinfo.price);
        }
    }

    productProfitInfoList.forEach((p) => delete p.price);

    return productProfitInfoList;
}
